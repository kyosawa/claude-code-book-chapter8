...
**受け入れ条件**:
- [x] `task add "タイトル"` でタスクを作成でき、一意のIDが自動採番される
- [x] `task list` で全タスクをID・ステータス・タイトル・紐付きブランチ形式で一覧表示できる
- [x] `task show <id>` でタスクの全詳細(タイトル・ステータス・説明・作成日・ブランチ)を確認できる
- [x] `task done <id>` でタスクを完了状態に変更できる（in_progress のタスクのみ対象）
- [x] `task delete <id>` で確認プロンプトの後にタスクを削除できる
- [x] タスクデータは `.task/tasks.json` に保存され、コマンド実行後も永続化される
...
**受け入れ条件**:
- [x] タスクは `open`(新規)・`in_progress`(作業中)・`completed`(完了)・`archived`(アーカイブ)の4ステータスを持つ
- [x] `task list --status <status>` でステータス別にフィルタリングして表示できる
- [x] ステータス変更は自動(task start/done/archive コマンドで遷移)に対応する（直接ステータス指定コマンドはP1以降で対応予定）
...
**受け入れ条件**:
- [x] `task start <id>` を実行すると `feature/task-<id>-<タイトルのslug>` ブランチが自動作成される
- [x] ブランチ作成後、自動でそのブランチに切り替わる
- [x] `task list` の表示に現在タスクに紐付いたブランチ名が表示される
- [x] Gitリポジトリが存在しない環境では、ブランチ連携なしでタスク管理のみ動作する
...
**受け入れ条件**:
- [x] `task list` の出力フォーマットが `ID | Status | Title | Branch` の列形式で表示される（Priority・Due列も追加）
- [x] 1000件のタスクを1秒以内に表示できる（JSONのon-memory処理で対応）
- [x] ステータス別に色分け表示される(in_progress:黄色、completed:緑、open:白、archived:グレー)
